...
def test_collect_statistics_success_env_creds(mock_client_cls, mock_time, mock_send_cb):
    ...
    with patch.dict(
        "os.environ",
        {"CONFLUENCE_BASE_URL": "url", "CONFLUENCE_EMAIL": "e", "CONFLUENCE_API_TOKEN": "t"},
    ):
...
def test_collect_statistics_vault_creds(mock_client_cls, mock_vault, mock_send_cb):
    ...
    with patch.dict("os.environ", {}):
        out = confluence._collect_confluence_statistics_impl(
            MagicMock(), _fake_event(space_key="ZZZ")
        )
    assert out["status"] == "success"
...
def test_collect_statistics_workererror_causes_retry(mock_client_cls, mock_send_cb):
    class DummySelf:
        ...
        def retry(self, exc):
            raise Exception(f"retrying: {exc}")
    ...
    with patch.dict(
        "os.environ",
        {"CONFLUENCE_BASE_URL": "u", "CONFLUENCE_EMAIL": "e", "CONFLUENCE_API_TOKEN": "t"},
    ):
        try:
...
def test_collect_statistics_workererror_callback_fails(mock_client_cls, mock_send_cb):
    class DummySelf:
        ...
        def retry(self, exc):
            raise Exception("shouldn't retry")
...
def test_get_credentials_from_vault_success(mock_hvac):
    ...
    client.secrets.kv.v2.read_secret_version.return_value = {
        "data": {"data": {"base_url": "U", "email": "E", "api_token": "T"}}
    }

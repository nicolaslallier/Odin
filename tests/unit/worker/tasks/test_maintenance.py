...
@patch("asyncio.run")
def test_cleanup_old_logs_no_db(monkeypatch, mock_run):
...
@patch("asyncio.run")
def test_cleanup_old_logs_async_error(monkeypatch, mock_run):
...
@patch("asyncio.run")
def test_cleanup_old_logs_env_override(monkeypatch, mock_run):
    base_env(monkeypatch, postgres_dsn="postgresql+asyncpg://foo/bar")
...
def test_log_statistics_dsn_fallback(mock_run, monkeypatch):
    ...
    fake_stats = {"status": "success", "total_logs": 5, "table_size": "10 MB", "by_level": {}, "oldest_log": None, "newest_log": None, "timestamp": "2024"}
    mock_run.return_value = fake_stats
    result = maintenance.log_statistics_task.run()
    assert result["status"] == "success"
